...
const loginSchema = z.object({
  ...
  password: z
    .string()
    .min(8, '영문 숫자 포함 8글자 이상 입력하세요.')
    .regex(/[a-zA-Z]/, '영문 숫자 포함 8글자 이상 입력하세요.')
    .regex(/[0-9]/, '영문 숫자 포함 8글자 이상 입력하세요.'),
})
...
export default function LoginPage() {
  ...
  // 이메일 형식 + 비밀번호 규칙(영문·숫자 포함 8자 이상)을 모두 만족해야 버튼 활성화
  const canSubmit = loginSchema.safeParse(watch()).success
  ...
  if (status === 'authenticated') {
    return (
      <Navigate to={user?.hasTeam ? ROUTES.home : ROUTES.welcome} replace />
    )
  }
  ...
  return (
    <div className="font-sans flex min-h-screen justify-center bg-bg-secondary">
      <div className="flex min-h-screen w-full max-w-[1440px] items-center justify-center">
        {/* Left  */}
        <div className="hidden shrink-0 p-x10 lg:block">
          <div className="relative h-[810px] w-[560px] shrink-0 overflow-hidden rounded-2xl">
        ...
        {/* Right */}
        <div className="flex flex-1 flex-col items-center mx-auto py-x10">
          <div className="flex w-[480px] flex-col gap-x10">
            ...
            <div className="flex flex-col gap-x5">
              ...
              <div className="flex flex-col gap-x10">
                ...
                >
                  <div className="flex flex-col gap-x5">
                    <InputField
                      ...
                      type="email"
                      autoComplete="email"
                      placeholder="ex) Loovi@gmail.com"
                      // 값이 비면 에러 대신 기본 상태로(입력 후 지웠을 때 error 잔류 방지)
                      errorMessage={
                        watch('email').trim()
                ...
                <div className="flex flex-col gap-x5">
                  ...
                  <LoginButton provider="google" interactive={false} />
                  <LoginButton provider="naver" interactive={false} />
                </div>
              </div>
            </div>
          </div>
